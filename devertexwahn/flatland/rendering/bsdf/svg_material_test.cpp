...
TEST(SvgMaterial, SetGetStrokeColor) {
    PropertySet ps;
    auto stroke_color = ColorRGB3f{1.f, 0.5f, 0.1f};
...
TEST(SvgMaterial, SetGetFillColor) {
    auto fill_color = ColorRGB3f{1.f, 0.5f, 0.1f};
    PropertySet ps;
...
TEST(SvgMaterial, BSDF) {
    ...
    EXPECT_THAT(material.sample(bsdf_sample, Vector2f{.5f}), ColorRGB3f{0.f});
    EXPECT_THAT(material.pdf(bsdf_sample), 0.f);
    EXPECT_THAT(material.evaluate(bsdf_sample), ColorRGB3f{0.f});
}

TEST(SvgMaterial, WhenADefaultInitalized_ThenDefaultMaterialValues) {
    PropertySet ps;
    SvgMaterial material{ps};
    EXPECT_THAT(material.stroke_color(), (ColorRGB3f{0.f, 0.f, 0.f}));
    EXPECT_THAT(material.fill_color(), (ColorRGB3f{1.f, 1.f, 1.f})); // TODO: Check if brackets can be moved to macro - maybe impossible because of C++ syntax?
    EXPECT_THAT(material.stroke_width(), 1.f);
}

TEST(SvgMaterial, GivenAPropertySet_WhenInitializingMaterial_ExpectProperValues) {
    PropertySet ps;
    ps.add_property("stroke_color", ColorRGB3f(1.f, 1.f, 1.f));
    ps.add_property("stroke_width", 3.f);
    ps.add_property("fill_color", ColorRGB3f(1.f, 1.f, 1.f));

    SvgMaterial material(ps);
    EXPECT_THAT(material.stroke_color(), ColorRGB3f(1.f, 1.f, 1.f));
    EXPECT_THAT(material.fill_color(), ColorRGB3f(1.f, 1.f, 1.f));
