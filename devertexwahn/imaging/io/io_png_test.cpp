...
TEST(Image4f, convert_to_Image4f) {
    Image4b source_image{2, 2, ColorRGBA4b{0, 128, 255, 255}};
    store_png("source.png", source_image);
    ReferenceCounted<Image4f> dest_image = load_image_png_as_Image4f("source.png");
    EXPECT_THAT(dest_image->get_pixel(0,0), (ColorRGBA4f{0.f, 128.0f/255.f, 1.f, 1.f}));
}
...
TEST(Image, load_RGBA_png_file) {
    ...
    EXPECT_THAT(image->get_pixel(0,0), (ColorRGB3f{1.f, 1.f, 0.f}));
    EXPECT_THAT(image->get_pixel(255,0), (ColorRGB3f{0.f, 1.f, 0.f}));
    EXPECT_THAT(image->get_pixel(0,255), (ColorRGB3f{1.f, 0.f, 0.f}));
    EXPECT_THAT(image->get_pixel(255,255), (ColorRGB3f{0.f, 0.f, 1.f}));
}
...
TEST(Image, load_image_png_as_Image4b) {
    ...
    for(int y = 0; y < image->height(); ++y) {
        for(int x = 0; x < 15; ++x) {
            EXPECT_THAT(image->get_pixel(x,y), (ColorRGBA4b{1, 102, 177, 255}));
        }
    }
    ...
    for(int y = 0; y < image->height(); ++y) {
        for(int x = 20; x < image->width(); ++x) {
            EXPECT_THAT(image->get_pixel(x,y), (ColorRGBA4b{255, 255, 255, 255}));
        }
    }
}
...
TEST(Image, load_png_file) {
    ...
    for(int y = 0; y < image->height(); ++y) {
        for(int x = 20; x < image->width(); ++x) {
            EXPECT_THAT(image->get_pixel(x,y), (ColorRGB3f{1.f, 1.f, 1.f}));
        }
    }
}
